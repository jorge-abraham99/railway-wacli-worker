# Build Railway wacli → Supabase Bridge

## Goal

Modify the existing Railway worker repo so it runs:

1. `wacli` as a persistent WhatsApp sync worker.
2. A local Node.js webhook bridge.
3. The bridge receives live `wacli` message webhooks.
4. The bridge upserts those messages into Supabase.

The Railway service already has:

- A persistent volume mounted at `/data`
- `WACLI_STORE_DIR=/data/wacli`
- A valid WhatsApp session already stored in `/data/wacli/session.db`
- A local wacli SQLite database at `/data/wacli/wacli.db`

Do **not** run `wacli auth` again. The service is already authenticated.
...
wacli sync --follow
↓
POST http://localhost:8787/wacli
↓
Node bridge
↓
...
Create a package.json file in the repo root:
...
Create a bridge.js file in the repo root:
...
Replace the existing start.sh with:
